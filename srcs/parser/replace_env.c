...
/*   Updated: 2023/01/11 15:41:16 by hajeong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	replace_env_exit_status(t_list *l_tok)
{
	...
	while (l_tok != NULL)
	{
		while (l_tok != NULL && l_tok->label >= AFTER_HEREDOC)
			l_tok = l_tok->next;
		if (l_tok != 0 && l_tok->label != 2 && ft_strnstr(l_tok->content, "$?", ft_strlen(l_tok->content)))
		{
			key = ft_strchr(l_tok->content, '$');
			key_end = key + 2;
		...
		if (l_tok && (!(ft_strnstr(l_tok->content, "$?", ft_strlen(l_tok->content)))|| l_tok->label == 2))
			l_tok = l_tok->next;
	}
}
